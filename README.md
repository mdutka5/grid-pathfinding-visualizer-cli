...
**Grid Pathfinding Visualizer** is a CLI utility for running and comparing state-space search algorithms on 2D grids. It processes `.txt` problem instances and outputs a step-by-step visualization of the search process from Initial Node to Goal Node and the final path.
